...
def generate_random_filename():
    common_names = [
        "main", "app", "script", "program", "code", "example", "test", "sample", "demo", "project"
    ]
    return random.choice(common_names) + ".py"
...
while time.time() < end_time:
    ...
    # Генерация случайных координат для перемещения мыши
    random_x = random.randint(0, pyautogui.size().width)
    random_y = random.randint(0, pyautogui.size().height)
